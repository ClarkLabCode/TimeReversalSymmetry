...
def main(_):
    ...
    log_file = 'joblist_natural_phase_randomized.txt'
    with open(log_file, 'w') as f:
        f.truncate()
        for R in range(Repeat):
            for D_cnn in D_cnn_list:
                for C in C_list:
                    f.write(f'module load miniconda; source activate py3_pytorch; python3 train_validate_run.py --config=../configs/configs_model_natural_phase_randomized.py --D_cnn={D_cnn} --C={C} --R={R+1}\n')
