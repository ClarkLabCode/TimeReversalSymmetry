...
def main(_):
    ...
    log_file = 'joblist_natural_inverted_test.txt'
    with open(log_file, 'w') as f:
        f.truncate()
        for R in range(Repeat):
            f.write(f'module load miniconda; source activate py3_pytorch; python3 test_run.py --config=../configs/configs_model_natural_inverted.py --R={R+1}\n')
