...
folder_nat = folder_main + 'panoramic/data_natural_only_phase_randomized/'
if not os.path.exists(folder_nat):
    os.makedirs(folder_nat)
folder_nat_filtered = folder_main + 'panoramic/data_natural_only_filtered_phase_randomized/'
...
for item in tqdm(items):
    if int(item[-22:-18]) in goodScenes_new:
        ...
        img_ifft = hpfn.get_phase_randomized(img, delete_zero_freq=True)
        np.save(folder_nat + f'natural_{int(item[-22:-18])}', img_ifft)

        K_row = img_ifft.shape[0] 
        K_col = img_ifft.shape[1] 
