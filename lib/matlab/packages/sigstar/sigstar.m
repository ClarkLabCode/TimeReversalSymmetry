function varargout=sigstar(groups,stats,transpose, nosort)
    ...
    for ii=1:length(groups)
        if transpose == 0
            ...
            thisY = findMinX(xlocs(ii, :)) + yd;
        end

        H(ii,:)=makeSignificanceBar(xlocs(ii,:),thisY,stats(ii), transpose);
    end
    %-----------------------------------------------------
...
function H=makeSignificanceBar(x,y,p, transpose)
    ...
    if transpose == 0
        ...
        H(1)=plot(y, x(:),'-k','LineWidth',1.5,'Tag','sigstar_bar');
    end
    ...
    if transpose == 0
        ...
        starY=mean(y)+myRange(xlim)*offset;
    end
    if transpose == 0
        ...
        'Tag','sigstar_stars', 'fontsize', font_size, 'interpreter', interpreter, 'Rotation', 0);
    else
        H(2)=text(starY, mean(x(:)), stars,...
        'HorizontalAlignment','Center',...
        'VerticalAlignment', 'middle',...
        'BackGroundColor','none',...
        'Tag','sigstar_stars', 'fontsize', font_size, 'interpreter', interpreter, 'Rotation', -90);
    end
